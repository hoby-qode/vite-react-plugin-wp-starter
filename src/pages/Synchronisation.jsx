...
import ButtonSynchroLocalSeries from './../components/ButtonSynchroLocalSeries'
...
function DataTable({ columns, data, onChangeData }) {
  ...
  const handleStatus = async (id) => {
    try {
      const res = await axios.post(
        `http://localhost/wordpress/wp-json/hqfastservice/v1/change-status-product`,
        { productId: id },
      )
      if (res.status <= 200) {
      ...
      <div className="flex align-middle mb-3 mt-5 justify-between">
        <div className="actions bulkactions">
          ...
          <select name="action" id="bulk-action-selector-top">
            ...
            <option value="make-all-dispo" className="hide-if-no-js">
              Disponible
            </option>
          </select>
          <input
        ...
        <ButtonSynchroLocalSeries onChangeData={onChangeData} prevData={data} />
      </div>
                  ...
                  <TableCell>
                    ...
                    <label htmlFor={`cb-select-${row.original.id}`}>
                      <span className="screen-reader-text">
                        Select {row.original.post_title}{' '}
                      </span>
                    </label>
                    <div className="locked-indicator">
                      ...
                      ></span>
                      <span className="screen-reader-text">
                        {row.original.post_title} is locked{' '}
                      </span>
                    </div>
                  </TableCell>
                    ...
                    <TableCell key={cell.id}>
                      {key == 0 ? (
                        <img
                          ...
                          alt={cell.row.original.post_title}
                          width={80}
                        />
                      ) : key == 2 ? (
                        <div
                          dangerouslySetInnerHTML={{
                            __html: cell.row.original.post_content.slice(
                              0,
                              150,
                            ),
                          }}
                        ...
                        cell.row.original.categories.map((cat, key) => (
                          <Badge
                            key={key}
                            variant="outline"
                        ...
                        cell.row.original.tags.map((tag, key) => (
                          <Badge
                            key={key}
                            variant="secondary"
                            className={clsx('mr-2 mb-2')}
